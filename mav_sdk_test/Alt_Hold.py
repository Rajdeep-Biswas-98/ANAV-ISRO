...
# async def wait_for_altitude(drone, target_alt, percent=0.85):
#     threshold = target_alt * percent
#     print(f"[WAIT] Waiting to reach at least {threshold:.2f}m altitude ({percent*100:.0f}% of target)...")
#     async for position in drone.telemetry.position():
...
async def run():
    ...
    await wait_for_altitude(drone, 1)
